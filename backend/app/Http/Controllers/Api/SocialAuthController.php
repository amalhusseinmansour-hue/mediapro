...
class SocialAuthController extends Controller
{
    ...
    private function getOAuthUrl(string $platform, string $state): ?string
    {
        ...
        return match($platform) {
            ...
            'linkedin' => sprintf(
                'https://www.linkedin.com/oauth/v2/authorization?client_id=%s&redirect_uri=%s&scope=%s&response_type=code&state=%s',
                config('services.linkedin.client_id'),
                urlencode($callbackUrl),
                urlencode('openid profile email w_member_social'),
                $state
            ),
