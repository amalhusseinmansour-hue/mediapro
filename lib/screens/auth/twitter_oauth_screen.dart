...
/// شاشة OAuth لربط حساب Twitter
class TwitterOAuthScreen extends StatefulWidget {
  const TwitterOAuthScreen({super.key});

  @override
  State<TwitterOAuthScreen> createState() => _TwitterOAuthScreenState();
}

class _TwitterOAuthScreenState extends State<TwitterOAuthScreen> {
  ...
  static const String _callbackUrl = 'https://mediaprosocial.io/api/twitter/callback';
  ...
  Future<void> _getAuthUrl() async {
    try {
      ...
      final response = await apiService.getTwitterAuthUrl();
  ...
  void _initWebView() {
    ...
    print('Twitter Auth URL: $_authUrl');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          throw Exception(data['error'] ?? 'Failed to connect Twitter');
        }
      } else {
