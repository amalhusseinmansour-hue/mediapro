...
/// شاشة OAuth لربط حساب YouTube
class YouTubeOAuthScreen extends StatefulWidget {
  const YouTubeOAuthScreen({super.key});

  @override
  State<YouTubeOAuthScreen> createState() => _YouTubeOAuthScreenState();
}

class _YouTubeOAuthScreenState extends State<YouTubeOAuthScreen> {
  ...
  static const String _callbackUrl = 'https://mediaprosocial.io/api/youtube/callback';
  ...
  Future<void> _getAuthUrl() async {
    try {
      ...
      final response = await apiService.getYouTubeAuthUrl();
  ...
  void _initWebView() {
    ...
    print('YouTube Auth URL: $_authUrl');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          throw Exception(data['error'] ?? 'Failed to connect YouTube');
        }
      } else {
