...
/// شاشة OAuth لربط حساب Instagram Business
class InstagramOAuthScreen extends StatefulWidget {
  const InstagramOAuthScreen({super.key});

  @override
  State<InstagramOAuthScreen> createState() => _InstagramOAuthScreenState();
}
...
class _InstagramOAuthScreenState extends State<InstagramOAuthScreen> {
  ...
  static const String _callbackUrl = 'https://mediaprosocial.io/api/instagram/callback';
  ...
  Future<void> _getAuthUrl() async {
    try {
      ...
      final response = await apiService.getInstagramAuthUrl();
  ...
  void _initWebView() {
    ...
    print('Instagram Auth URL: $_authUrl');
  ...
  NavigationDecision _handleNavigation(String url) {
    ...
    // Check for cancel/access denied
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          throw Exception(data['error'] ?? 'Failed to connect Instagram');
        }
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFFE4405F), // Instagram gradient color
        elevation: 0,
        leading: IconButton(
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'يجب أن يكون لديك حساب Instagram Business\nمرتبط بصفحة Facebook',
                      style: TextStyle(
                        color: Colors.grey[400],
                        fontSize: 12,
                      ),
